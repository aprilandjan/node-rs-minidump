...
pub fn get_craspad_info(path: String) -> napi::Result<CrashpadInfo> {
  ...
  let dump = match dump_result {
    Ok(dump) => dump,
    Err(_) => return Err(Error::new(Status::InvalidArg, "read minidump file failed".to_owned()))
  };
  ...
  let crashpad_info = match crashpad_info_result {
    Ok(info) => info,
    Err(_) => return Err(Error::new(Status::GenericFailure, "get crashpadInfo stream from dump file failed".to_owned()))
  };
